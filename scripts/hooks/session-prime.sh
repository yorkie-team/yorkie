...
- Branch from main. `make verify` (lint, licence headers, unit tests) green
  per commit; the pre-commit and pre-push hooks check this for you.
...
- Never hand-edit api/yorkie/v1/*.pb.go — change the .proto, `make proto`.
EOF
