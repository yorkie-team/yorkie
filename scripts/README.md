...
| `go-fix.sh` | `scripts/go-fix.sh check`, or `make verify-modernize`; `scripts/go-fix.sh apply`, or `make modernize` | Runs the toolchain's `go fix` modernizers over every build tag, not just the default build that `go fix ./...` sees. The tag set is read off the `//go:build` lines, so a new tag cannot fall outside it, and the tree is analysed as linux/amd64. `check` fails when a rewrite is pending or when the tree does not compile under those tags; `apply` rewrites until nothing is left. It also refuses a negated tag or a platform other than linux/amd64, which turning every tag on cannot cover. `make verify` runs `check`, and so does CI's `modernize` lane. |
