...
// NewClient creates and returns a new webhook Client configured with a default
// http.Client. The returned Client does not hold any per-instance retry or
// timeout options; per-call behavior (timeouts, retries, wait intervals) is
// driven by the Options passed to Send.
...
// createSignature computes the HMAC-SHA256 of data using hmacKey and returns it formatted
// for use in an `X-Signature-256` header as `sha256=<hex>`.
// If hmacKey is empty this returns an empty string and no error.
// An error is returned only if the underlying hash write fails.
