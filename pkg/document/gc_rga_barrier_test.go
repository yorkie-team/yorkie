...
func runConcurrentMovesScenario(t *testing.T, collect bool) {
	...
	// What is left is held back, not leaked, and it is held back identically on
	// both sides: the insert-created slot of "a", which every later ArraySet on
	// "a" still names as its anchor. It is released when "a" is itself removed
	// and that removal goes stable. Retaining the same thing on both replicas is
	// what matters here -- an asymmetric retention would mean the two are
	// deciding collection from different information.
	require.Equal(t, dB.GarbageLen(), dA.GarbageLen(), "the replicas retain different amounts")
	require.Equal(t, 1, dA.GarbageLen(), "only the moved element's insert slot is held back")
}
