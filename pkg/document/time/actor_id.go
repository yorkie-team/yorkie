...
// ActorIDFromBase64 decodes a base64-encoded string into an ActorID.
func ActorIDFromBase64(str string) (ActorID, error) {
	if str == "" {
		return ActorID{}, fmt.Errorf("%s: %w", str, ErrInvalidActorID)
	}
	...
	if len(decoded) != actorIDSize {
		return ActorID{}, fmt.Errorf("decoded length %d: %w", len(decoded), ErrInvalidActorID)
	}
...
// ToBase64 encodes the ActorID into a base64 string.
func (id ActorID) ToBase64() string {
