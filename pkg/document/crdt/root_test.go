...
func TestRoot(t *testing.T) {
	...
	t.Run("garbage collection for text test", func(t *testing.T) {
		...
		_, pairs, _, err := text.Edit(fromPos, toPos, "Hello World", nil, ctx.IssueTimeTicket(), nil)
		assert.NoError(t, err)
		...
		_, pairs, _, err = text.Edit(fromPos, toPos, "Yorkie", nil, ctx.IssueTimeTicket(), nil)
		assert.NoError(t, err)
		registerGCPairs(root, pairs)
	...
	t.Run("garbage collection for fragments of text", func(t *testing.T) {
		...
		for _, tc := range steps {
			...
			_, pairs, _, err := text.Edit(fromPos, toPos, tc.content, nil, ctx.IssueTimeTicket(), nil)
			assert.NoError(t, err)
			registerGCPairs(root, pairs)
