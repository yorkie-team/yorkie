...
func getActivatedClients(t *testing.T, n int) (clients []*Client) {
	for i := 0; i < n; i++ {
		c, err := NewClient(testYorkie.RPCAddr())
		assert.Nil(t, err)
...
func cleanupClients(t *testing.T, clients []*Client) {
...
func TestClientNewAndClose(t *testing.T) {
	cli, err := NewClient(testYorkie.RPCAddr())
	assert.Nil(t, err)
...
func TestClientActivateAndDeactivate(t *testing.T) {
	cli, err := NewClient(testYorkie.RPCAddr())
	assert.Nil(t, err)
...
func TestClientAndDocument(t *testing.T) {
	...
	t.Run("attach/detach test", func(t *testing.T) {
		...
		if err := c1.Attach(ctx, doc); err != nil {
			t.Error(err)
		}
		assert.True(t, doc.IsAttached())

		if err := c1.Detach(ctx, doc); err != nil {
			t.Error(err)
		}
	...
	t.Run("causal nested array test", func(t *testing.T) {
		...
		if err := c1.Attach(ctx, doc1); err != nil {
			t.Error(err)
		}
		...
		if err := c2.Attach(ctx, doc2); err != nil {
			t.Error(err)
		}
	...
	t.Run("watch test", func(t *testing.T) {
		...
		go func() {
			...
			if err := c1.Sync(ctx, resp.Keys...); err != nil {
				t.Error(err)
			}
		}()
