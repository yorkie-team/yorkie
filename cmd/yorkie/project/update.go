...
// newUpdateCommand returns a Cobra command that updates a project by name.
// It loads authentication and RPC configuration, retrieves the current project,
// applies flag-driven changes (name, auth/event webhook settings including URLs, methods/events,
// retry and timeout policies, client deactivate threshold, subscriber/attachment limits,
// and remove-on-detach), sends the resulting UpdatableProjectFields to the admin API,
// reports any server-side BadRequest field violations, and prints the updated project
// in the configured output format (JSON or YAML).
...
// init registers the "update" subcommand and its CLI flags for updating project settings.
// It configures flags for project name, authorization and event webhook URLs, methods/events to add or remove, webhook retry/backoff/timeouts, client deactivate threshold, document subscriber/attachment limits, and remove-on-detach, then adds the command to SubCmd.
func init() {
