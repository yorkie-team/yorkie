...
// init registers custom validation rules and their error translations used by
// UpdatableProjectFields. It adds validators for:
// - "invalid_webhook_method": ensures each entry is a valid auth method,
// - "invalid_webhook_event": ensures each entry is a valid event type,
// - "duration": ensures a string parses as a time.Duration,
// - "valid_origin": accepts "*" or a valid URL.
//
// If any registration or translation fails, the function writes the error to
// stderr and exits the process with a non-zero status.
func init() {
