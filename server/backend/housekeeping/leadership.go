...
func NewLeadershipManager(db database.Database, nodeID string, conf *LeadershipConfig) *LeadershipManager {
	if conf == nil {
...
func (lm *LeadershipManager) Start(ctx context.Context) error {
	...
	if logger := logging.From(ctx); logger != nil {
		logger.Info("leadership manager started", "node_id", lm.nodeID)
	}
	return nil
}
...
func (lm *LeadershipManager) tryAcquireLeadership(ctx context.Context) error {
	lease, err := lm.database.TryLeadership(ctx, lm.nodeID, "", lm.leaseDuration)
	if err != nil {
		return fmt.Errorf("acquire leadership: %w", err)
	}

	if lease.NodeID == lm.nodeID {
...
func (lm *LeadershipManager) renewLease(ctx context.Context) error {
	...
	lease, err := lm.database.TryLeadership(ctx, lm.nodeID, currentLease.LeaseToken, lm.leaseDuration)
	if err != nil {
