...
func CreateKeySpace(conf *Config) error {
	...
	if conf.Port != 0 {
		sysCluster.Port = conf.Port
	}
	sysCluster.Keyspace = "system"
	sysCluster.Consistency = gocql.One
	sysCluster.Timeout = gotime.Second * 5
	sysCluster.ProtoVersion = 4
	// Match DialSession: skip peer discovery so the session only talks to the
	// configured hosts. Without this, gocql tries to reach all cluster nodes
	// via their broadcast addresses (e.g., docker-internal 172.x IPs), which
	// hangs from the host's network namespace.
	sysCluster.DisableInitialHostLookup = true
...
func DialSession(conf *Config) (*Session, error) {
	...
	if err := createTables(context.Background(), session, conf.Keyspace, conf.Tables); err != nil {
		session.Close()
