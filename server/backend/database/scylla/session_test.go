...
// setupTestWithDummyData dials a scylla.Client backed by a real ScyllaDB +
// MongoDB pair, with every table group enabled on ScyllaDB. The keyspace is
// randomized per call so sub-tests do not see each other's data.
func setupTestWithDummyData(t *testing.T) *scylla.Client {
	...
	assert.NoError(t, mongoConf.Validate())

	scyllaConf := scylla.GetScyllaConfig()
	scyllaConf.Keyspace = "yorkie_test_" + scylla.RandomKeyspace()
	scyllaConf.Tables = scylla.Tables{
		Clients:        true,
		Changes:        true,
		Snapshots:      true,
		VersionVectors: true,
	...
	cli, err := scylla.Dial(scyllaConf, mongoConf)
