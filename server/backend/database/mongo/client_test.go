...
func TestClient(t *testing.T) {
	...
	t.Run("single document IsDocumentAttached test", func(t *testing.T) {
		...
		assert.NoError(t, cli.UpdateClientInfoAfterPushPull(ctx, c1, d1))
		attached, err = cli.IsDocumentAttached(ctx, dummyProjectID, d1.ID)
		assert.NoError(t, err)
		...
		assert.NoError(t, cli.UpdateClientInfoAfterPushPull(ctx, c2, d1))
		attached, err = cli.IsDocumentAttached(ctx, dummyProjectID, d1.ID)
		assert.NoError(t, err)
		assert.True(t, attached)
	...
	t.Run("two documents IsDocumentAttached test", func(t *testing.T) {
		...
		assert.NoError(t, cli.UpdateClientInfoAfterPushPull(ctx, c1, d2))
		attached, err = cli.IsDocumentAttached(ctx, dummyProjectID, d2.ID)
		assert.NoError(t, err)
		assert.True(t, attached)
