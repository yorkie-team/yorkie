...
func (c *Client) FindChangeInfosBetweenServerSeqs(
	...
	// Check if we have the complete range in store
	if store.HasCompleteRange(from, to) {
		return store.ChangesInRange(from, to), nil
	}

	// Fetch missing ranges from database
	for _, rg := range store.CalculateMissingRanges(from, to) {
		...
		// Create a new range for the store
		if len(infos) > 0 {
			store.AddRange(&ChangeRange{
				From:      rg.From,
				To:        rg.To,
				Changes:   infos,
