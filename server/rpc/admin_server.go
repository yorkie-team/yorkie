...
func newAdminServer(
	...
	return &adminServer{
		...
		clusterSem:        semaphore.NewWeighted(int64(be.Config.MaxConcurrentClusterRPCs)),
	}
}
...
func (s *adminServer) GetChannels(
	...
	for firstPath, keys := range groups {
		...
		go func() {
			...
			if err := s.clusterSem.Acquire(ctx, 1); err != nil {
				resultCh <- groupResult{err: err}
				return
			}
			defer s.clusterSem.Release(1)
