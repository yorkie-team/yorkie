...
// DocAttachKey generates a sync key for the document attachment.
func DocAttachKey(docKey types.DocRefKey) sync.Key {
	return sync.NewKey(fmt.Sprintf("doc-attach-%s-%s", docKey.ProjectID, docKey.DocID))
}

// DocWatchKey generates a key for watching document.
func DocWatchKey(docID types.ID, clientID time.ActorID) sync.Key {
	return sync.NewKey(fmt.Sprintf("doc-watch-%s-%s", docID, clientID))
}

// DocEditKey generates a sync key for the document edit.
func DocEditKey(docKey types.DocRefKey) sync.Key {
	return sync.NewKey(fmt.Sprintf("doc-edit-%s-%s", docKey.ProjectID, docKey.DocID))
}
...
func pushPack(
	...
	locker := be.Lockers.Locker(DocEditKey(docKey))
	defer func() {
