...
Found while building the Tree `Style` reverse operation. **Present identically
in JS, and traced to a specific PR, so not fixed** -- per this document's
usual rule.

`TreeStyleOperation.execute` (`tree_style_operation.ts:127-158`) is
`if (this.attributes.size) { ... tree.style(...) ... } else { ... const
attributesToRemove = this.attributesToRemove; ... tree.removeStyle(...) ...
}` -- an if/else, not two independent branches. `StyleOperation.execute`
(Text's twin, `style_operation.ts:125-166`) is two independent `if`
blocks, one for `attributesToRemove` and one for `attributes`, both run
whenever their field is non-empty. Go's `operations.Style.Execute`
(`pkg/document/operations/style.go`) ports Text's independent-if shape;
`operations.TreeStyle.Execute` (`pkg/document/operations/tree_style.go`)
ports Tree's if/else shape -- each faithful to its own JS source, which is
why the two Go operations read differently for what looks like the same
job.

The difference is invisible for every *forward* call: `json/tree.go`'s
`Tree.Style` and `Tree.RemoveStyle` (mirroring
`packages/sdk/src/document/crdt/tree.ts`'s json-level callers) always
construct a `TreeStyleOperation`/`TreeStyle` with exactly one of the two
fields populated, so only one branch ever has anything to do regardless of
if/else vs independent-if. It surfaces only when `Execute` runs a
**combined** reverse -- one built by `toReverseOperation` with both a
restore (`attributes`) and a removal (`attributesToRemove`) at once, which
happens whenever a single `Style` call touches a mix of a key that already
had a value and a key that did not exist before. On JS's if/else, `if
(this.attributes.size)` is true, so `tree.style()` runs and restores the
existing key(s) -- but the `else` branch, and with it `tree.removeStyle()`
for the newly-added key(s), never runs. The key that should be removed is
left in place. This applies to that reverse's every execution: the
undoing client's own redo, and any peer that receives the reverse (or a
redo of it) over the wire.

### Provenance

`git log` on both files pins this precisely. Text's independent-if shape
was introduced by PR #1174 ("Add undo/redo support for Text.Style
operations"). Tree's undo/redo support followed two months later in PR
#1221 ("Add undo/redo support for TreeStyleOperation"), whose commit
message says the reverse-op logic follows "Text's StyleOperation pattern"
-- and the reverse-*building* code (the final `if
(reversePrevAttributes.size > 0 && reverseAttrsToRemove.length > 0) ...`
block) does match Text's exactly. But the surrounding forward-execute
branch was left as Tree's pre-existing if/else rather than being converted
to Text's independent-if shape alongside it. The combined-reverse
constructor `TreeStyleOperation` now hands out is a shape its own
`execute` cannot fully replay -- an oversight introduced by #1221 copying
half of #1174's pattern, not a deliberate design difference between Text
and Tree.

### Reachability

Any single `Style` call whose range covers a key that already carries a
value and a key that does not, e.g. bold already `"true"`, style with
`{bold: "false", italic: "true"}`. Ordinary single-client editing reaches
this easily; it does not need concurrency the way several other entries in
this document do.
...
- [ ] Decide the fix: convert `TreeStyleOperation.execute`'s if/else to two
      ...
      `CRDTTree.style`/`removeStyle`'s independent signatures (verified
      neither takes the other's parameter, so nothing else depends on the
      exclusivity)
- [ ] Fix in `yorkie` and `yorkie-js-sdk` together, or add a version gate,
      per this document's usual rule for cross-SDK behavior changes
- [ ] Once fixed, tighten the pinning tests that currently assert the
      broken behavior back to asserting full restore-and-removal:
      `pkg/document/operations/tree_style_test.go`'s "reverse of a mixed
      set carries both branches, but only restores on execute" and
      "reverse of a mixed set survives a wire round trip", and
      `pkg/document/tree_style_undo_test.go`'s "style undo unions restore
      and removal across keys test"
