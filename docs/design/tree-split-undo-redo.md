...
| Deep-copied boundary nodes keep their original IDs, so a re-insertion can put two nodes under one `CRDTTreeNodeID` | Realized in production (yorkie#1927): a position anchored at such an ID resolved to a different node after a snapshot rebuild and split past the end of its value. The server now drops content that reuses an earlier change's ID and keeps the live node registered for it — see [tree](tree.md#one-node-per-treenodeid). Split and merge reverses are exactly the cases that still take the copy path rather than identity-preserving restore, so this doc's flows are the ones that keep exercising it. |
