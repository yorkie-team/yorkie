...
| Apply | `change.execute(this.root, ...)` → `{opInfos, operations, reverseOps}` | `c.Execute(d.doc.root, ...)` → `error` before this port; `(change.ExecutionResult, error)` after it, where the result carries `Executed`, `Observable` and `ReverseOps` |
...
| Caller | Role | Uses |
|---|---|---|
| `Document.Update` | local edit, real root | `Executed` → `ReconcileCreatedAt`, `ReverseOps` → undo stack, `Observable` → the redo-stack clear |
| `Document.executeUndoRedo` | undo/redo, clone then real root | clone call discards the result; real-root call uses `Observable` for the empty-change early return and `ReverseOps` for the opposite stack |
...
| `admin.Client.ListChangeSummaries` | replays a document's own stored changes into a snapshot-seeded `InternalDocument`, one change at a time, real root | discards both; runs under `OpSourceReplay` via `InternalDocument.ApplyChangesForReplay`. Missed in the pass that added the two rows above it: this loop ran under `OpSourceRemote` via `ApplyChanges` until it was found, paying the full Tree and Text reverse bookkeeping on every stored change for a caller that reads neither return value — the row above's "`executed` → reconciliation, on a client" was true of every caller only until this one turned up, and `OpSourceReplay` was never exclusive to `ApplyChangePack` either, contrary to what the pre-PR fix report first said |
...
| Undoing a newly-introduced presence key: Go's `ReversePresence` sent the zero value (`""`) for a key absent from the snapshot; JS's `undefined` is dropped by `JSON.stringify` before the wire, removing the key instead. A genuine Go/JS divergence, and wire-visible | **Fixed** in CodeRabbit review. `ReversePresence` now reports absent keys separately, `HistoryOperation` carries them as `PresenceAbsentKeys`, and `executeUndoRedo` applies `presence.Presence.Delete` for each — Go's spelling of JS's `set({key: undefined})`, since a `map[string]string` has no `undefined` to assign. The characterization test that pinned `""` now pins the removal |
| A *partially* skipped undo/redo still ships the skipped operation. Each SDK decides whether to propagate from what the execution reported (`len(executed) == 0` in `Document.executeUndoRedo`; `!opInfos.length` in `document.ts:2145`) — two gates that agree on a *skipped* operation, which enters neither list; Go's gate is now `Observable` too (see the next row) — while the `Change` pushed to `localChanges` (`Document.executeUndoRedo`; `document.ts:2149`) carries every operation `ctx.Push`/`ctx.push` added, skipped or not. A 3-op undo where op #2 hits `ErrOperationSkipped` still ships intact; peers execute op #2 under `OpSourceRemote`, where the skip guard (`isRemovedOrOrphaned`) never runs. JS-identical, so correct as a port — but Critical 1's fix and `TestHistorySkippedUndo` only close the *fully*-skipped case (nothing observable executed), not this partial one | Not fixed, not filed as its own task. Currently written down only here |
| `len(executed)` was not `opInfos`, and is no longer what either gate asks. Go's old gate meant "the operation ran without erroring"; JS's means "the operation produced an observable change". They diverge on an operation that runs and produces nothing. Probed with a genuine no-op edit (`text.Edit(0, 0, "")`), which JS's `rga_tree_split.ts` reports as zero `changes` (an empty insert is skipped by `if (value)`, an empty delete range never satisfies `fromIdx < toIdx`): **(a)** undoing it shipped an empty change in Go, spending a clientSeq and a change-log row where JS returns early at `document.ts:2145`; **(b)** the no-op edit itself cleared the redo stack in Go where JS gates `clearRedo` on `opInfos.length` (`document.ts:768`) and keeps it, so Go reported `CanRedo() == false` and JS `true` | **Fixed.** `Operation.Execute` now returns an `operations.ExecutionResult` carrying `Observable` alongside `Reverse`, `Change.Execute` folds it into `change.ExecutionResult.Observable`, and both gates read that instead of `len(executed)`. `Edit` derives it exactly as JS derives its OpInfos — content inserted (`rga_tree_split.ts:665`) or a live node removed (`rga_tree_split.ts:1564`) on the ordinary path, and un-tombstoned/recreated/re-tombstoned nodes on the identity-preserving one. Every other operation reports `true`: that is what Go did before the field existed, and reporting `false` where JS would emit an OpInfo would silently drop a real edit, whereas reporting `true` where JS would emit nothing only costs a redundant change. `Style`, `TreeEdit` and `TreeStyle` derive their OpInfos from per-node change lists in JS that the Go CRDT layer does not report, so they stay conservative — the residual over-report, and the only part of this row that still ships. Covered by `a no-op edit keeps the redo stack test` and `undoing a no-op edit queues no change test` in `pkg/document/history_test.go`. The residual over-report is filed in [20260816-remote-redo-replica-divergence-todo.md](../tasks/active/20260816-remote-redo-replica-divergence-todo.md) |
