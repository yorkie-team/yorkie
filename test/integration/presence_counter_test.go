...
func TestPresenceCounterIntegration(t *testing.T) {
	...
	t.Run("multiple clients presence counter test", func(t *testing.T) {
		...
		// Detach one client
		err = client2.Detach(ctx, counter2)
		require.NoError(t, err)
		assert.False(t, counter2.IsAttached())

		// Detach remaining clients
		err = client1.Detach(ctx, counter1)
		require.NoError(t, err)
		assert.False(t, counter1.IsAttached())

		err = client3.Detach(ctx, counter3)
		require.NoError(t, err)
	...
	t.Run("out-of-order event handling test", func(t *testing.T) {
		...
		// Rapid attach/detach operations to potentially create out-of-order events
		var wg sync.WaitGroup
		for i := 0; i < 5; i++ {
			wg.Add(1)
			go func() {
				defer wg.Done()
				_ = clients[1].Detach(ctx, counter2)
				time.Sleep(10 * time.Millisecond) // Small delay
				_ = clients[1].Attach(ctx, counter2)
			}()
		}

		// Monitor counts for a period - should see valid transitions only
		timeout := time.After(3 * time.Second)
		var lastValidCount int64 = 2
		validTransitions := true

		for {
			select {
			case count := <-countCh:
				// Count should only be 1 or 2, and transitions should make sense
				if count != 1 && count != 2 {
					validTransitions = false
					t.Errorf("Invalid count received: %d", count)
				}
				// Log valid transitions for debugging
				if count != lastValidCount {
					t.Logf("Count transition: %d -> %d", lastValidCount, count)
		...
		// Final verification
		require.True(t, validTransitions, "All count transitions should be valid")

		// Give a moment for final state to stabilize
		time.Sleep(100 * time.Millisecond)

		// Final stable count should be 2 (both attached)
		// Use a helper function to get the latest count from the channel
		getFinalCount := func() int64 {
			// Drain any pending counts and return the latest
			var finalCount int64 = lastValidCount
			for {
				select {
				case count := <-countCh:
					finalCount = count
					t.Logf("Final count update: %d", count)
				default:
					return finalCount
				}
			}
		...
		assert.Eventually(t, func() bool {
			...
			t.Logf("Checking final count: %d", finalCount)
			return finalCount == 2
	...
	t.Run("concurrent count consistency test", func(t *testing.T) {
		...
		for i := range clientCount {
			go func(idx int) {
				err := clients[idx].Attach(ctx, counters[idx])
	...
	t.Run("mixed document and presence test", func(t *testing.T) {
		clients := activeClients(t, 1)
		defer deactivateAndCloseClients(t, clients)
