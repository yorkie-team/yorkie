...
func TestObject(t *testing.T) {
	...
	t.Run("object.set with json literal test", func(t *testing.T) {
		...
		assert.Equal(t, 10, d1.GarbageCollect(helper.MaxVectorClock(d1.ActorID())))
		assert.Equal(t, 10, d2.GarbageLen())
		assert.Equal(t, 10, d2.GarbageCollect(helper.MaxVectorClock(d2.ActorID())))
	})
	...
	t.Run("object.set with json literal array type test", func(t *testing.T) {
		...
		assert.Equal(t, 3, d1.GarbageCollect(helper.MaxVectorClock(d1.ActorID())))
	})
