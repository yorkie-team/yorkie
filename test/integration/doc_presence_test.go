...
func TestDocPresence(t *testing.T) {
	...
	t.Run("presence-related events test", func(t *testing.T) {
		...
		wrch, cancel1, err := c1.WatchStream(d1)
		assert.NoError(t, err)
		defer cancel1()
		go func() {
			for {
	...
	t.Run("watch after detach events test", func(t *testing.T) {
		...
		_, cancel2, err := c2.WatchStream(d2) // No events yet - before the window time for batch publishing
		assert.NoError(t, err)
		defer cancel2()
		assert.NoError(t, c2.Detach(ctx, d2))
		assert.NoError(t, c1.Sync(ctx))
	...
	t.Run("GetDocument presence filtering test", func(t *testing.T) {
		...
		// 03. WatchDocStream both clients to the document
		_, cancel1, err := c1.WatchStream(d1)
		assert.NoError(t, err)
		defer cancel1()
		_, cancel2, err := c2.WatchStream(d2)
		assert.NoError(t, err)
