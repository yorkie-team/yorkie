...
func TestTree(t *testing.T) {
	...
	t.Run("auxiliary test", func(t *testing.T) {
		ctx := context.Background()
		d1 := document.New(helper.TestDocKey(t))
		assert.NoError(t, c1.Attach(ctx, d1))
		d2 := document.New(helper.TestDocKey(t))
		...
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1abcd45fgh23</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1abcd45fgh23</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		// TODO: need to generate & check path comparison like js-sdk
		assert.NoError(t, d2.Update(func(root *json.Object, p *presence.Presence) error {
		...
		lastChangeOps := d2.InternalDocument().GetLastLocalChange().Operations()
		if len(lastChangeOps) > 0 {
			if treeEdit, ok := lastChangeOps[0].(*operations.TreeEdit); ok {
				fromPos := treeEdit.FromPos()
				fromParent, fromLeft, err := d2.Root().GetTree("t").FindTreeNodesWithSplitText(fromPos, lastChangeOps[0].ExecutedAt())

				if err == nil {
					fromPath, err2 := d2.Root().GetTree("t").ToPath(fromParent, fromLeft)
					if err2 == nil {
						assert.Equal(t, fromPath, []int{1, 0, 0, 4})
					}
				}
			}
		}
		...
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1abcd45gh23</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1abcd45gh23</n></p></c></r>", d2.Root().GetTree("t").ToXML())
	})

	// Concurrent editing, client reload, after edit test
	t.Run("concurrently edit with client reload case test", func(t *testing.T) {
		ctx := context.Background()
		d1 := document.New(helper.TestDocKey(t))
		assert.NoError(t, c1.Attach(ctx, d1))
		d2 := document.New(helper.TestDocKey(t))
		...
		assert.NoError(t, d1.Update(func(root *json.Object, p *presence.Presence) error {
			...
			root.GetTree("t").EditByPath([]int{1, 0, 0, 3}, []int{1, 0, 0, 3}, &json.TreeNode{
				Type:  "text",
				Value: "네이버랑 ",
			}, 0)
			return nil
		...
		assert.NoError(t, c1.Sync(ctx))
		assert.NoError(t, c2.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>12 네이버랑 3</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>12 네이버랑 3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, d2.Update(func(root *json.Object, p *presence.Presence) error {
			root.GetTree("t").EditByPath([]int{1, 0, 0, 1}, []int{1, 0, 0, 8}, &json.TreeNode{
				Type:  "text",
				Value: " 2 네이버랑 ",
			}, 0)
			...
			root.GetTree("t").EditByPath([]int{1, 0, 0, 2}, []int{1, 0, 0, 3}, &json.TreeNode{
				Type:  "text",
				Value: "카",
			}, 0)
			...
			root.GetTree("t").EditByPath([]int{1, 0, 0, 3}, []int{1, 0, 0, 4}, &json.TreeNode{
				Type:  "text",
				Value: "캉",
			}, 0)
			...
			root.GetTree("t").EditByPath([]int{1, 0, 0, 4}, []int{1, 0, 0, 5}, &json.TreeNode{
				Type:  "text",
				Value: "올",
			}, 0)
			...
			root.GetTree("t").EditByPath([]int{1, 0, 0, 5}, []int{1, 0, 0, 6}, &json.TreeNode{
				Type:  "text",
				Value: "랑",
			}, 0)
		...
		assert.NoError(t, c2.Sync(ctx))
		assert.NoError(t, c1.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오랑 2 네이버랑 3</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오랑 2 네이버랑 3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, d1.Update(func(root *json.Object, p *presence.Presence) error {
			root.GetTree("t").EditByPath([]int{1, 0, 0, 13}, []int{1, 0, 0, 14}, nil, 0)
			root.GetTree("t").EditByPath([]int{1, 0, 0, 12}, []int{1, 0, 0, 13}, nil, 0)
		...
		assert.NoError(t, c1.Sync(ctx))
		assert.NoError(t, c2.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오랑 2 네이버3</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오랑 2 네이버3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, d2.Update(func(root *json.Object, p *presence.Presence) error {
			root.GetTree("t").EditByPath([]int{1, 0, 0, 6}, []int{1, 0, 0, 7}, nil, 0)
			root.GetTree("t").EditByPath([]int{1, 0, 0, 5}, []int{1, 0, 0, 6}, nil, 0)
		...
		assert.NoError(t, c2.Sync(ctx))
		assert.NoError(t, c1.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오2 네이버3</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오2 네이버3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, d1.Update(func(root *json.Object, p *presence.Presence) error {
			root.GetTree("t").EditByPath([]int{1, 0, 0, 9}, []int{1, 0, 0, 10}, nil, 0)
		...
		assert.NoError(t, c1.Sync(ctx))
		assert.NoError(t, c2.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오2 네이3</n></p></c></r>", d1.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카카오2 네이3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		c3 := activateSingleClient(t)
		...
		assert.NoError(t, c2.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카2 네이3</n></p></c></r>", d3.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 카2 네이3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, d3.Update(func(root *json.Object, p *presence.Presence) error {
			root.GetTree("t").EditByPath([]int{1, 0, 0, 2}, []int{1, 0, 0, 3}, nil, 0)
		...
		assert.NoError(t, c3.Sync(ctx))
		assert.NoError(t, c2.Sync(ctx))
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 2 네이3</n></p></c></r>", d3.Root().GetTree("t").ToXML())
		assert.Equal(t, "<r><c><u><p><n></n></p></u></c><c><p><n>1 2 네이3</n></p></c></r>", d2.Root().GetTree("t").ToXML())

		assert.NoError(t, c3.Deactivate(context.Background()))
